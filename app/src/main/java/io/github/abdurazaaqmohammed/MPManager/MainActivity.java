...
public class MainActivity extends AppCompatActivity {
    ...
    private void showShizukuGuideOnce(File folder, boolean pane1) {
        ...
        if (!installed) {
            ...
            message = rss.getString(R.string.shizuku_not_running);
    ...
    public void setCurrentFolder(File curr, File[] files) {
        ...
        if (files == null) this.<TextView>findViewById(R.id.folderCount).setText(rss.getString(R.string.folders_files_x, 0, 0));
            ...
            int foldersCount;
            if (ShizukuFile.isAndroidDataPath(curr)) {
                // ShizukuFile folders can't listFiles(); count from the entries we already have.
                foldersCount = 0;
                for (File f : files) if (f.isDirectory()) foldersCount++;
            } else {
                File[] folders = curr.listFiles(File::isDirectory);
                foldersCount = folders == null ? 0 : folders.length;
            }
