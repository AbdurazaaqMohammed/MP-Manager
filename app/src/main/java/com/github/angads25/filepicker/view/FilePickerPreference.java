...
public class FilePickerPreference extends Preference implements
    ...
    private void initProperties(AttributeSet attrs) {
       try(TypedArray typedArray=getContext().getTheme().obtainStyledAttributes(attrs, R.styleable.FilePickerPreference,0,0)) {
           final int N = typedArray.getIndexCount();
           for (int i = 0; i < N; ++i)
           {   int attr = typedArray.getIndex(i);
               if (attr == R.styleable.FilePickerPreference_selection_mode) {
                   properties.selection_mode=typedArray.getInteger(R.styleable.FilePickerPreference_selection_mode, DialogConfigs.SINGLE_MODE);
               }
               else if (attr == R.styleable.FilePickerPreference_selection_type) {
                   properties.selection_type=typedArray.getInteger(R.styleable.FilePickerPreference_selection_type,DialogConfigs.FILE_SELECT);
               }
               else if (attr == R.styleable.FilePickerPreference_root_dir) {
                   String root_dir=typedArray.getString(R.styleable.FilePickerPreference_root_dir);
                   if(!TextUtils.isEmpty(root_dir))
                   {   properties.root=new File(root_dir);
                   }
               }
               else if (attr == R.styleable.FilePickerPreference_error_dir) {
                   String error_dir=typedArray.getString(R.styleable.FilePickerPreference_error_dir);
                   if(!TextUtils.isEmpty(error_dir))
                   {   properties.error_dir=new File(error_dir);
                   }
               }
               else if (attr == R.styleable.FilePickerPreference_offset_dir) {
                   String offset_dir=typedArray.getString(R.styleable.FilePickerPreference_offset_dir);
                   if(!TextUtils.isEmpty(offset_dir))
                   ...
                   titleText=typedArray.getString(R.styleable.FilePickerPreference_title_text);
               }
           }
       }
    }
